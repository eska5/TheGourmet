...
def test_meals_success(client, mocker):
    ...
    resp = client.post('/meals', json={"mealName": example_name, "mealPhoto": example_photo})
    assert resp.status_code == 201
    save_image_mock.assert_called_once_with(title=example_name, coded_image=example_photo,
                                            path=m_operator.NEW_IMAGES_PATH)
...
def test_bad_result(client, mocker):
    ...
    save_image_mock = mocker.patch("app.m_operator.save_image")
    resp = client.post('/badresult', json={"useroutput": example_correct_name, "modeloutput": example_bad_name,
                                           "mealPhoto": example_photo})
    assert resp.status_code == 201
    save_image_mock.assert_called_once_with(title=mock_bad_result_name, coded_image=example_photo,
